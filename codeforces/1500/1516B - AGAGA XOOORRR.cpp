/*
    ...
    Submission At : 2026-09-03 17:13:09
*/
...
void solve() {
    ...
    loop(i, 0, n-1){
        ...
        if(currXor == totalXor){ //First group we have found then second like this
            cnt++;
            currXor = 0;
        }
    }

    if(cnt >= 2){ //If minimum 2 group xor same from total then all is same element
        yes();
    }
    else{
        no();
