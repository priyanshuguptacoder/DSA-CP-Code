/*
    ...
    Submission At : 2026-09-17 21:40:42
*/
...
ll ans(vi& arr, int n){
    sortAsc(arr);
    return 1LL * arr[n / 2] - arr[(n - 1) / 2] + 1; //When it will even then distance between both median and when odd then only one point that is median so like we can find this
}
