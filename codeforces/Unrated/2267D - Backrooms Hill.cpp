...
╚█████╔╝██║  ██║██║    ███████║██║  ██║██║  ██║███████╗███████╗    ██║  ██║██║  ██║██║ ╚═╝ ██║
 ╚════╝ ╚═╝  ╚═╝╚═╝    ╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝╚══════╝    ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝     ╚═╝
    ...
    Submission At : 2026-09-26 03:46:41
*/
...
void solve() {
    ...
    //In question it tell i and i+2 only swap so from this we can analyze that we have to use parity from this we can see even idx at even and odd at odd idx so this is concept we can use that the number which at even remain at even and which at odd remain at odd
    vi parity(n+1);
    loop(i, 0, n){
        parity[arr[i]] = i % 2;
    }
    ...
    int l = 0, r = n - 1;
    loop(i, 1, n+1){ //We can check every number
        if(l % 2 == parity[i]){
            l++;
        }
        else if(r % 2 == parity[i]){
            r--;
        }
        else{
            no();
