/*
    ...
    Submission At : 2026-09-14 21:44:29
*/
...
void solve() {
    ...
    loop(i, 1, n){ //Build prefix function
        int j = pi[i - 1];
        while(j > 0 && s[i] != s[j]){
            j = pi[j - 1];
        }
    ...
    int len = pi[n - 1]; //Longest prefix which is also a suffix
    
    int maxi = 0;
    loop(i, 0, n-1){ //Maximum border length apperaing before the last position
        maxi = max(maxi, pi[i]);
    }
