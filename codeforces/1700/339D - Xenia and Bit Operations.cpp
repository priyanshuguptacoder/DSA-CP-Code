/*
    ...
    Submission At : 2026-09-09 06:24:30
*/
...
void build(int node, int l, int r, bool isOr){
    ...
    build(left, l, mid, !isOr);
    build(right, mid+1, r, !isOr);
...
void update(int node, int l, int r, int pos, ll val, bool isOr){
    if(l == r){
        tree[node] = val;
        return ;
    }
    ...
    if(pos <= mid){
        update(left, l, mid, pos, val, !isOr);
    }
    else{
        update(right, mid+1, r, pos, val, !isOr);
    }
...
void solve() {
    ...
    //Root operations -> n odd or and n even XOR
    bool isOr = (n % 2 == 1);
    build(0, 1, N, isOr);
