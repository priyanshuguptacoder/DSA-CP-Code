/*
    ...
    Submission At : 2026-09-21 00:36:46
*/
...
void solve() {
    ...
    if(n > m){ //Swap n, m because we itearte over smaller dimension
        swap(n, m);
    }
    ...
    while(low < high){
        ...
        loop(i, 1, n+1){ //Count kar lo kitne chote hai <= mid
            cnt += min(m, mid / i); //Agar element jayde aa rahe ho aur uske row me value kam ho isliye minimum le rahe
        }

        if(cnt < k){ //Not enough elements <= mid
            low = mid + 1;
        }
        else{ //Mid can contain the k-th smallest
            high = mid;
        }
    }
