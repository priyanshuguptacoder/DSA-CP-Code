/*
    ...
    Submission At : 2026-09-06 01:43:30
*/
...
ll countPair(vll& arr, ll x, ll n){
    int i = 0;
    int j = n - 1;
    ...
    while(i < j){
        if(arr[i] + arr[j] <= x){
            cnt += (j - i); //Sort hai toh agaar chote wale se ho ja raha toh chote wale se bhi ho jayega
            i++;
        }
        else{
            j--;
...
void solve() {
    ...
    cout << countPair(arr, r, n) - countPair(arr, l - 1, n) << endl;
}
