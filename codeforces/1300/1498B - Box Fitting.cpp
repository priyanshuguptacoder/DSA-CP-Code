/*
    ...
    Submission At : 2026-08-31 22:45:53
*/
...
void solve() {
    ...
    multiset<ll> s; //We are using multiset baecuse it give sorted value automatically and upper bound in O(log n) we can used in multiset
    loop(i, 0, n){
        s.insert(arr[i]);
    }
    ...
    ll space = w;
    while(!s.empty()){
        auto it = s.upper_bound(space); //It returns first element greater than element
        if(it != s.begin()){
            --it;
            space -= *it;
            s.erase(it);
