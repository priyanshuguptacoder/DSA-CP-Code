/*
    ...
    Submission At : 2026-09-19 19:54:34
*/
...
void solve() {
    ...
    vll costs;
    for(ll &x : arr){ //Generate all possible +1 beauty upgrades
        while(true){
            ll cost = (~x) & (x + 1); //Lowest 0 bit this is the minimum cost to increase beauty by 1
            if(cost > k){
                break;
            }

            costs.push_back(cost);
            x += cost;  //Move x to the state after gaining +1 beauty.
        }
    }

    sortAsc(costs); // Every upgrade gives exactly +1 beauty so take the cheapest upgrades first.
    for(ll cost : costs){
        if(cost > k){
            break;
        }
