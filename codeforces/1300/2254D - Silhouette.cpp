/*
    ...
    Submission At : 2026-09-04 20:27:01
*/
...
void solve() {
    ...
    vector<pair<ll, int>> v; //Store (shadow value, original index)
    v.reserve(n);
    ...
    vll val(n); //Group equal shadow values
    int i = 0;
    ll prevVal = -1;
    ...
    while(i < n){
        ...
        ll currVal;
        if(j < n){
            //Next Shadow = currShadow + cnt * cntVal;
            ll nextSha = v[j].first;
            ll diff = nextSha - currSha;
        ...
        if(currVal <= 0 || (prevVal != -1 && currVal <= prevVal)){
            cout << -1 << endl;
            return ;
        }

        loop(k, i, j){
            val[v[k].second] = currVal;
        }
        prevVal = currVal;
        i = j;
    }
