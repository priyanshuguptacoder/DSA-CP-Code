/*
    ...
    Submission At : 2026-09-07 23:23:07
*/
...
void solve() {
    ...
    vi pref(n); //Maximum number of characters from beginning match with t from s
    int j = 0;
    ...
    vi suff(n); //Maximum number of characters from ending match with t from s
    j = m - 1;
    loopb(i, n-1, 0){
        suff[i] = m - 1 - j;
    ...
    vector<vector<int>> pos(26); //Store position of t in pos
    loop(i, 0, m){
        pos[t[i] - 'a'].push_back(i);
    }

    loop(i, 0, n){ //Check every position of s
        int l = m - 1 - suff[i];
        int r = pref[i];
        auto it = lower_bound(pos[s[i] - 'a'].begin(), pos[s[i] - 'a'].end(), l); //Find first occurenece of s[i] in t at position >= l
        if(it == pos[s[i] - 'a'].end() || *it > r){
            cout << "No" << endl;
            return ;
        }
    }
